...
import { Link, useSearchParams, useNavigate } from 'react-router-dom'; // Adicionado useSearchParams e useNavigate
...
const ProjectCard = ({ project, delay }) => {
  ...
  // Calcula progresso (Placeholder - Lógica real virá do back-end ou cálculo local)
  const calculateProgress = (status) => {
    // Mapeamento simples de status para progresso
    const statusMap = {
      'Planejamento': 10,
      'Design': 30,
      'Desenvolvimento': 60,
      'Testes': 80,
      'Lançado': 100,
      'Manutenção': 100, // Considerado "concluído" em termos de progresso inicial
      'Cancelado': 0
    };
    // Tenta encontrar o status no mapa, senão retorna 0
  ...
  // Status Badge (pode ser um componente separado)
  let statusBgColor = 'bg-blue-700';
  let statusTextColor = 'text-blue-100';
  // Define cores com base no status do projeto
  ...
  return (
    <motion.div
      className="bg-gray-800 p-5 rounded-lg border border-gray-700 shadow-md flex flex-col justify-between hover:border-red-500/50 transition-colors duration-200"
      {...fadeIn(delay)}
    >
      <div>
        {/* Nome e Status */}
      ...
      <div className="mt-auto border-t border-gray-700 pt-3 text-xs text-gray-500 space-y-1.5">
        ...
        <Link
          ...
          to={`#`} // Temporariamente aponta para nada
...
const ClientProjects = () => {
  ...
  const [searchParams, setSearchParams] = useSearchParams(); // Hook para ler/escrever filtros na URL
  const navigate = useNavigate(); // Para erro 401

  // Lê o filtro da URL (ex: ?filter=ativos) ou usa 'ativos' como padrão
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        ...
        // NOTA: Idealmente, a API aceitaria o filtro: /api/projects/myprojects?status=ativos
        // Como ainda não implementamos isso, buscamos todos.
   ...
   // Filtra os projetos no front-end baseado no estado 'currentFilter'
  // useMemo evita recalcular a cada renderização se 'projects' ou 'currentFilter' não mudarem
  const filteredProjects = useMemo(() => {
    // Retorna todos os projetos se o carregamento ainda não terminou
    if (loading) return []; 
    
    return projects.filter(p => {
      // Define os status correspondentes a cada filtro
      const activeStatus = ['Planejamento', 'Design', 'Desenvolvimento', 'Testes'];
      const completedStatus = ['Lançado'];
      const maintenanceStatus = ['Manutenção']; // Adiciona status Manutenção

      // Aplica o filtro baseado no valor de 'currentFilter'
      ...
      // Se o filtro for desconhecido, retorna false para não mostrar nada inesperado
      return false; 
    });
  }, [projects, currentFilter, loading]); // Adiciona loading como dependência

  // Função para mudar o filtro na URL
  const handleFilterChange = (newFilter) => {
    setSearchParams({ filter: newFilter }); // Atualiza o parâmetro 'filter' na URL
  };

  // Botões de Filtro - Configuração
  const filterButtons = [
  ...
  return (
    <div>
          ...
          <motion.div className="flex space-x-2" {...fadeIn(0.1)}>
             {filterButtons.map(btn => (
                <button
                  ...
                  // Estilo dinâmico para botão ativo/inativo
                  className={`px-3 py-1.5 text-xs sm:text-sm font-medium rounded-md transition-colors duration-150 ${
                      currentFilter === btn.key
                      ? 'bg-red-600 text-white shadow-sm' // Botão ativo
                      : 'bg-gray-700 text-gray-300 hover:bg-gray-600' // Botão inativo
                  }`}
                >
      ...
      {/* Mensagem se não houver projetos após carregar e filtrar */}
